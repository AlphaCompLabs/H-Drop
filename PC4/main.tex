...
% PC3 — H-DROP: Módulo logístico autônomo de entrega
% ============================================================
...
% ============================================================
\title{\textbf{Automação e Robótica -- Relatório Técnico de Projeto [PC3]}\\
\vspace{0.5cm}
...
% ============================================================
\section{Resumo executivo do estágio atual do projeto}

O projeto H-DROP consiste no desenvolvimento de um Veículo de Superfície Autônomo (ASV) do tipo catamarã, concebido para atender demandas de logística humanitária e comercial em regiões de difícil acesso terrestre. Desde o Ponto de Controle 2 (PC2), o projeto avançou significativamente da fase de integração em bancada para a validação física do sistema completo. A estrutura em PVC foi montada, o teste de flutuabilidade foi realizado com sucesso, os motores e ESCs foram integrados ao chassi, e a bateria LiFePO4 foi validada em conjunto com o módulo BMS. Os motores foram testados com a bateria do projeto, confirmando a viabilidade do subsistema de propulsão.

No âmbito do \textit{firmware}, o sistema opera sobre FreeRTOS com uma Máquina de Estados Finita (FSM), e os subsistemas de telemetria 4G/GNSS via MQTT e a Estação de Controle de Missão em Angular foram consolidados. Os próximos passos até o PC4 compreendem a calibração do controlador PID em ambiente aquático, os ensaios de navegação autônoma ponto a ponto, a validação do sistema anticolisão em movimento e a coleta de dados de desempenho para análise final.
...
\textbf{Evolução desde os pontos de controle anteriores:} No PC1, o projeto estava em fase puramente conceitual, com modelagem matemática dos subsistemas, definição de requisitos e proposta arquitetural. No PC2, a transição para a prática revelou desafios reais: queima de um ESP32 por falha de soldagem, limitação do sensor ultrassônico único e dificuldades de manufatura do chassi em PVC. Esses desafios foram endereçados com correções técnicas documentadas. No PC3, o protótipo físico existe, flutua, e tem seus principais subsistemas elétricos e de \textit{firmware} validados em bancada e em ensaios preliminares.
...
\subsection{Etapas Executadas no PC3}
...
% ============================================================
\section{Avanços e Resultados Parciais}
...
\subsubsection{Planejamento da Torre de Controle}
Como ajuste crítico de projeto para o PC3 e elemento de integração entre o hardware de radiofrequência, o firmware e a estrutura física, foi projetado o acoplamento de uma \textbf{Torre de Controle Vertical} em PVC rígido na embarcação. O planejamento desta estrutura justifica-se pelos seguintes requisitos técnicos de integração:

\begin{enumerate}
    \item \textbf{Isolamento Eletromagnético da Bússola:} O magnetômetro QMC5883L (I2C nos pinos GPIO21/SDA e GPIO22/SCL) exige isolamento contra campos magnéticos parasitas gerados pelos motores brushless e pelo cabeamento de 12V. A torre eleva o sensor verticalmente, mitigando o efeito \textit{hard-iron} e garantindo a precisão do cálculo de proa ($\theta = \operatorname{atan2}(Y,X)$).
    \item \textbf{Desimpedimento do Plano de Terra RF:} A antena ativa de GNSS e a antena de chicote 4G LTE foram posicionadas no topo da torre, desimpedidas da lâmina d'água e de superfícies reflexivas, otimizando o \textit{Time-to-First-Fix} (TTFF) para manter estabilidade com 14 a 16 satélites rastreados.
    \item \textbf{Passagem de Cabeamento e Estanqueidade:} O interior cilíndrico da torre servirá como duto para a fiação blindada de sinal e alimentação, que será soldada definitivamente e direcionada para a caixa estanque IP67 inferior por meio de prensa-cabos, eliminando falhas por mau contato mecânico sob vibração.
\end{enumerate}
...
O teste de flutuabilidade confirma que a estrutura física é viável, mas ainda requer os flutuadores de EVA para atingir a margem de projeto. A integração dos motores valida a compatibilidade mecânica e elétrica do sistema de propulsão, mas a calibração da malha PID em ambiente aquático, etapa essencial para a missão autônoma, permanece como trabalho a concluir no PC4.

A Estação de Controle e o \textit{firmware} de telemetria atingiram os requisitos de latência e persistência definidos. A queima do ESP32 no PC2 foi corrigida com isolamento termorretrátil em todos os terminais expostos e revisão do barramento I2C, e não se repetiu nos testes subsequentes.

A dessincronização de dados entre o \textit{firmware} e o \textit{backend} foi resolvida com padronização da API sob o padrão \texttt{/api/v1/} e testes de \textit{loopback} via \textit{broker} HiveMQ. A instabilidade do 4G causada por picos de corrente de até 2{,}8~A foi mitigada pelos capacitores de tântalo, com publicação estabilizada a 2~Hz.
...
O H-DROP avançou do modelo puramente conceitual para um protótipo físico funcional. A estrutura em PVC foi montada, os ensaios de flutuabilidade foram concluídos com sucesso, e os principais componentes, bateria com BMS, motores, ESCs e \textit{firmware} de telemetria, foram validados em bancada. O sistema de comunicação 4G/GNSS opera de forma estável, com a Estação de Controle recebendo telemetria em tempo real.

As principais conquistas do PC3 são: (1) validação hidrostática da plataforma física; (2) integração mecânica e elétrica dos motores e ESCs ao chassi; (3) validação do subsistema de energia com BMS ativo; e (4) consolidação do \textit{firmware} com FreeRTOS, PID embarcado e telemetria MQTT funcional.

As limitações ainda existentes dizem respeito à calibração do PID em ambiente aquático real, à execução da missão autônoma completa de ponto A a ponto B. A reserva de flutuabilidade ainda está abaixo da meta de projeto de 40\% sem os flutuadores de EVA.

Para o PC4, as ações previstas incluem: testes de malha fechada (PID) na água com calibração dos ganhos $K_p$, $K_i$ e $K_d$; execução da missão autônoma de navegação por \textit{waypoints}; validação do sistema anticolisão em movimento; coleta e análise dos dados de desempenho energético e de navegação; e elaboração do relatório final de validação.
...
\section{Link dos códigos do GIT HUB}
https://github.com/AlphaCompLabs/H-Drop.git
